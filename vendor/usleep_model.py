"""
VENDORED THIRD-PARTY CODE - do not edit except to keep it importable.

U-Sleep model definition, taken verbatim (bar the two import shims below) from
uSLEEPYLAND, the U-Time fork whose published checkpoints this project loads:

    https://github.com/biomedical-signal-processing/uSLEEPYLAND
    utime/models/usleep.py

uSLEEPYLAND and SLEEPYLAND are MIT licensed (Copyright (c) 2024 Luigi Fiorillo).
The architecture is U-Sleep / U-Time by Perslev et al., cited in the original
docstring below.

Kept as a faithful copy on purpose: the checkpoints are weights-only, so the
architecture has to match the original exactly or the weights load into the
wrong places. Two `utime` package imports are replaced with local equivalents so
the file stands alone - compute_receptive_fields was only used for a logging
line, and get_activation_function resolves a string to a Keras activation.
"""
"""
...
# --- local stand-ins for the two `utime` imports (see header) ----------------
def compute_receptive_fields(layers):
    """Stub: the original is used only for an informational log line.

    Indexed as compute_receptive_fields(...)[-1][-1], and the result is then
    indexed again as receptive_field[0], so the innermost value has to be
    subscriptable too.
    """
    return [[[None]]]
...
def get_activation_function(activation_string):
    """Resolve an activation name the way Keras does."""
    import keras
    return keras.activations.get(activation_string)
